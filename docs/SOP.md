...
```mermaid
graph TD
    A[Phase 1: 双线并行基材准备] --> B[Phase 2: Router 战略提案]
...
### Phase 2：Router 战略提案（需确认 #1 — CRITICAL_STOP）

- **Agent**：01_Router
- **前置依赖**：`spu_fetched` + `keyword_snapshot_ready` 均已收到
- **动作**：读取飞书输入字段 + 关键词快照 → 生成 `SPU_CONTEXT` YAML → 写入战略/平台/赛道/视觉方向字段
- **闸门**：**CRITICAL_STOP** — Router 提案后必须等待人工说"确认"才能继续
- **超时**：300s 无确认 → 主控通知用户
- **产出**：`proposal_ready` 事件

---

### Phase 3：SEO to Listing 初版文案（需确认 #2 — HUMAN_CONFIRM）

- **Agent**：02_SEO_to_Listing (单一 Agent 处理 Amazon/Etsy/eBay 三平台)
- **动作**：读取 `SPU_CONTEXT` + 关键词快照 → 经 keyword-grader 取词/冻结快照 → 生成三平台初版文案
...
- **产出**：`draft_done` 事件（含 `platform` 字段标识是哪个平台）
...
- **Agent**：03_Visual
- **动作**：
  1. 读取初版 + VisualBridge → 生成终版标题/五点/描述
  2. 生成视觉 Prompt（Amazon/Etsy/eBay 各 Img1~7）
  3. 生成 A+ Copy/Prompt (01~10)
  4. **可选**：调用 Agnes AI 文生图/图生图/文生视频生成预览素材
- **新增**：遵循 `knowledge-base/video-templates/video_script_standard.md` 标准化视频脚本
- **产出**：`visual_final` 事件（含视觉 Prompts、A+ 内容、Agnes 生成物 URL）
...
- **风险分级**：
  ...
  - 🟠 **二级（高危/平台红线）** → 标注 ⚠️ + 给出替代词，**需用户确认"替换"**后回写
  - 🟡 **三级（中危/风格建议）** → 静默替换 / 标注建议，不阻塞流程
- **闸门**：**COMPLIANCE_CONFIRM** — 若有二级风险需用户确认替换
- **产出**：写入飞书 `合规扫描报告` + `合规状态`；发布 `compliance_check_result` / `risk_hit`
...
- **Agent**：04_Ads
- **动作**：读取初版标题/ST/痛点 → 生成 PPC 方案
- **产出**：写入 `Amazon_广告方案` 字段
