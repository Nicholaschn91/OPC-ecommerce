...
- 未获用户**逐条明确授权**，禁止对�飞书 Base A/B �� 做任何写操作、结构变更、字段增删。
- 绝对禁止：
  1. 直接 `DELETE` 任何 `type=18` 同表自关联字段（其级联会清空全表关联数据，�灾难级）；
  2. 结构变更前�飞书历史记录未开启 + 用户未逐条点头；
  3. �� 擅自建视图 / 字段 / � 层级。
- 任何"清理残留字段"、"�搭�骨架"、"试一下"类操作，必须先 `git pull` 最新�协议 + 向用户逐项确认，否则一律不做。

*本�协议由主控 Agent 维�护。修改需经用户 (Nicholas) 确认后�执行。*