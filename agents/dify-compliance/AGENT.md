...
```json
{
  ...
  "platforms_to_scan": ["amazon", "etsy", "ebay"],
  "scan_scope": "visual_final",
  "copy_fields": {
    "amazon": {"title": "...", "bullets": [...], "description": "...", "st": "...", "faq": [...]},
    "etsy":   {"title": "...", "tags": [...], "description": "..."},
    "ebay":   {"title_matrix": [...], "bullets": [...], "item_specifics": {...}, "desc_html": "..."}
  },
  "visual_prompts": {
    "amazon": {"img1": "...", "img2": "...", ...},
    "etsy":   {...},
...
| 产出 | 目标 | 说明 |
|------|------|------|
| `合规扫描报告` | 飞书字段 | JSON 格式完整报告 |
| `合规状态` | 飞书字段 | 单选：`通过` / `需修正` / `熔断` |
| 文案字段回写 | 飞书对应字段 | **仅**二级风险经用户确认后自动替换 |
...
Boss 收到 visual_final + 用户"确认"
    ▼
Boss 组装载荷 → 发布 compliance_check_request
    ▼
...
Boss 决策：
  ...
  - overall_status == "需修正"     → 等待用户 COMPLIANCE_CONFIRM → 回写文案
...
| **二级（高危）** | 绝对化用语、无资质"医疗级/有机/环保"、平台特定违规（ST 重复/超长、Tags 重复） | 标注 ⚠️ + 给出替代词，**需用户确认"替换"**后自动回写对应飞书字段 |
...
| 技能 | 用途 |
|------|------|
| `dify-compliance` | 封装 Dify API 调用、载荷组装、结果解析、飞书回写 |
