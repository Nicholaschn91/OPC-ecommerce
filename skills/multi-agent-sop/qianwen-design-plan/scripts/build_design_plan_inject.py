...
browser-qwen MCP `browser_run_code_unsafe` 的 code 参数执行。
...
# browser-qwen MCP 的 browser_run_code_unsafe 只允许读取其 output 根目录下的文件，
# 传工作区路径会报 "File access denied"。此处自动探测该根目录并拷贝载荷过去。
MCP_ROOT_GLOB = (
    "C:/Users/nicho/.workbuddy/logs/mcp-runtime/"
    "custom-mcp_browser-qwen-*/.playwright-mcp"
)


def detect_mcp_root():
    """返回最新的 browser-qwen MCP 允许根目录，找不到返回 None。"""
...
def main():
    ...
    ap.add_argument('--mcp-root', default='auto',
                    help="拷贝载荷到 browser-qwen MCP 允许根目录："
