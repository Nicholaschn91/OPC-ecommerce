...
- **真实浏览器，非 API**：必须通过 `mcp__aistudio__browser_*` 工具真实驱动 aistudio.google.com 网页端里的 **Nano Banana 2 Lite**（`gemini-3.1-flash-lite-image`）模型生成图片。
- **仅认 MCP**：所有浏览器操作只用 `mcp__aistudio__browser_*` 工具。**禁止手搓 Playwright 脚本**（全局铁律）。
...
| 项 | 值 |
|---|---|
| MCP | `aistudio`（复用 `aistudio-google-profile`，含已登录 Google 账号 `leiyuzhe007@gmail.com`（Nicholas Lei））|
...
> `ai_studio_*.py` 的设计是 **CLI 生成 MCP 命令 JSON**，由执行方照 JSON 逐条调用 `mcp__aistudio__browser_*` 工具。下方"黄金流程"是这些 CLI 产出的、已实测验证的命令本身——直接照做，或用 CLI 生成后再执行均可。
...
> 文件选择器若被 MCP 卡住，可改用 `mcp__aistudio__browser_file_upload` 直接传路径（参数 `paths` 必须是数组）。
...
```
> CLI 输出的是 **MCP 命令 JSON**，需由执行方逐条调用 `mcp__aistudio__browser_*` 落地。黄金流程（上节）即这些 CLI 的真实产出，二者等价。
...
- 环境：`aistudio` MCP 连接器（端口 9224，复用 `aistudio-google-profile`，登录账号 `leiyuzhe007@gmail.com`）
