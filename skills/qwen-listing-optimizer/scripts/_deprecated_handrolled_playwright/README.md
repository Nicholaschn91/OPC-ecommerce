...
> （如 `playwright-qwen` MCP）。**严禁手写或运行 Playwright/Chrome 自动化脚本**。
...
## 现行替代方案（见 SKILL.md）
浏览器全程走 **`playwright-qwen` MCP**，由 skill 的 MCP 驱动 recipe 控制：
...
## 登录态维护
登录态现由 `playwright-qwen` MCP 的 `cdp-profile-h` 承载。若 cookie 过期：
