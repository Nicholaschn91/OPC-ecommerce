...
# -*- coding: utf-8 -*-
"""
launch_cloak_qwen.py — 启动 CloakBrowser 隐身 Chromium 供 playwright-qwen MCP 连接（A 路线）

用途：让既有的 `playwright-qwen` MCP（mcp.json 里指向 127.0.0.1:9222）连上的是
...
def launch(headed):
    ...
    for _ in range(20):
        try:
            ...
            print("现在可在 WorkBuddy 用 mcp__playwright_qwen__* 工具（已隐身）")
            return
        except Exception:
            time.sleep(0.5)
